...
function Field({ label, value }: { label: string; value: string | null | undefined }) {
  return (
    <div>
      <p className="text-[0.65rem] text-muted-foreground tracking-wider uppercase font-medium mb-1">{label}</p>
      <p className="text-[0.88rem] text-near-black">{value || <span className="text-muted-foreground/60 italic">Not provided</span>}</p>
    </div>
  )
}
    ...
    <main className="min-h-screen bg-off-white">
      <div className="pt-28 pb-16 md:pt-36 md:pb-24">
        <div className="container-custom">
          <motion.div variants={stagger} initial="hidden" animate="visible" className="max-w-lg mx-auto">
            <motion.div variants={fadeInUp} className="mb-8">
              <Link href="/rsvp" className="inline-flex items-center gap-1.5 text-[0.78rem] text-muted-foreground hover:text-near-black transition-colors">
            ...
            <motion.h1 variants={fadeInUp} className="text-editorial text-2xl md:text-3xl text-near-black mb-8">
              Update Your RSVP
            </motion.h1>

            {lookupStatus !== 'found' && (
              <motion.form variants={fadeInUp} onSubmit={handleLookup} className="card p-6 md:p-8 space-y-5">
                <div>
                  <label htmlFor="lookup_email" className="block text-[0.68rem] text-muted-foreground mb-1.5 tracking-wider uppercase font-medium">
                    Your Email Address
                  </label>
                  <input
                    id="lookup_email"
              ...
              {lookupStatus === 'found' && record && (
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="card p-6 md:p-8">
                  ...
                  {!editing ? (
                    ...
                    <div className="space-y-4">
                      <div>
                        <label className="block text-[0.65rem] text-muted-foreground mb-1.5 tracking-wider uppercase font-medium">Full Name</label>
                      ...
                      <div>
                        <label className="block text-[0.65rem] text-muted-foreground mb-1.5 tracking-wider uppercase font-medium">School</label>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-[0.65rem] text-muted-foreground mb-1.5 tracking-wider uppercase font-medium">Grad Year</label>
                        ...
                        <div>
                          <label className="block text-[0.65rem] text-muted-foreground mb-1.5 tracking-wider uppercase font-medium">Major</label>
                      ...
                      <div>
                        <label className="block text-[0.65rem] text-muted-foreground mb-1.5 tracking-wider uppercase font-medium">LinkedIn URL</label>
                      ...
                      <div>
                        <label className="block text-[0.65rem] text-muted-foreground mb-1.5 tracking-wider uppercase font-medium">Dietary Restrictions</label>
                      ...
                      <div className="flex gap-3 pt-1">
                        ...
                        <button onClick={() => setEditing(false)} className="btn-secondary px-4">
                          Cancel
                        </button>
                      </div>

                      {saveStatus === 'error' && (
                        <p className="text-[0.8rem] text-destructive">Something went wrong. Please try again.</p>
                      )}
                    </div>
                  )}
                </motion.div>
