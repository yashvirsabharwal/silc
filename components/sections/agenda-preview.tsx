...
export function AgendaPreview() {
  return (
    <section id="agenda" className="section-padding bg-off-white border-y border-border">
      <motion.div
        initial="hidden"
        whileInView="visible"
        viewport={viewport}
          ...
          {agenda.map((block) => (
            ...
            >
              <div className="flex-shrink-0 w-20 md:w-24 pt-0.5">
                <span className="text-[0.75rem] font-mono text-gold font-medium">{block.time}</span>
              </div>
              <div className="flex-1">
                <h3 className="text-[0.88rem] text-near-black font-medium mb-1">{block.title}</h3>
                <p className="text-[0.8rem] text-muted-foreground leading-[1.65]">{block.description}</p>
              </div>
            </motion.div>
          ))}
        </motion.div>
